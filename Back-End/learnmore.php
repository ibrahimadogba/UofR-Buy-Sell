...
if (array_key_exists("Id", $_GET)) {
       $_SESSION["Id"] = $_GET["Id"];
        ...
        // check if the add to favorites button has been clicked
        if(isset($_POST["add_to_favorites"])) {
          ...
          $q3 = "INSERT INTO favorites (userId, sellId, created_dt ) VALUES (:userId, :sellId, NOW())";
          $stmt = $db->prepare($q3);
          $stmt->bindValue(':userId', $userId);
          $stmt->bindValue(':sellId', $sellId);
          if ($stmt->execute()) {
              echo '<script>alert("Added to favorites successfully!")</script>';
              echo '<script>window.location.href="learnmore.php";</script>';
          } else {
...
<body>
    ...
    <section id="prodetails" class="section-p1">
        ...
        <h6><strong>Books</strong></h6>
            ...
            <button class="normal" onclick="window.location.href='email.php?Id=<?=$row['sellId']?>'">Message the Seller</button>
            
            <h4>Product Details</h4>
            <form method="post">
            <button class="normal" name="add_to_favorites">Add to Favorites</button>
            </form>
            <h6></h6>
            <h6></h6>
            <h6></h6>
